...
$HostReport = @()
Get-VMHost |%{    
     $Report = "" | select Hostname, Cluster, version, Build, manufacture, Model,cpu_model, core_num, totalmemoryMB, P_nic
     $Report.Hostname = $_.Name
     $Report.Cluster = $_.Parent.Name
...
$ClusterReport | Export-Csv -NoTypeInformation -UseCulture -Path "DET-ClusterReport6_03_2013.csv"
$HostReport | Export-Csv –NoTypeInformation -useCulture -Path "DET-HostReport6_03_2013.csv" 
$VmInventory | Export-Csv -NoTypeInformation -UseCulture -Path "DET-VmInventory6_03_2013.csv"
