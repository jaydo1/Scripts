...
$Plugins = Get-ChildItem -Path $PluginsFolder
...
$MyReport = Get-CustomHTML "$VIServer vCheck"
	$MyReport += Get-CustomHeader0 ($VIServer)
	$Plugins | Foreach {
		$Details = . $_.FullName
		$ver = "{0:N1}" -f $Version
...
if ($Outputpath) {
	$Filename = $Outputpath + "\" + $VIServer + "vCheck" + "_" + $Date.Day + "-" + $Date.Month + "-" + $Date.Year + ".htm"
	$MyReport | out-file -encoding ASCII -filepath $Filename
}
...
if ($SendEmail) {
	...
	If ($SendAttachment) {
		send-Mailmessage -To $EmailTo -From $EmailFrom -Subject "$VISRV vCheck Report"  -SmtpServer $SMTPSRV -Body "vCheck attached to this email" -Attachments $Filename
	} Else {
		send-Mailmessage -To $EmailTo -From $EmailFrom -Subject "$VISRV vCheck Report"  -SmtpServer $SMTPSRV -Body $MyReport -BodyAsHtml
	}
}

if ($SendAsAttachment -eq $true -and $DisplaytoScreen -ne $true) {
